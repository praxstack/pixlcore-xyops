...
Page.Alerts = class Alerts extends Page.PageUtils {
	...
	do_bulk_export() {
		...
		var args = this.args;
		var query = this.getSearchQuery(args);
		var sopts = { index: 'alerts', query };
		var title = 'Bulk Export Alerts';
	...
	do_bulk_delete() {
		...
		Dialog.confirmDanger( 'Delete All Alerts', "Are you sure you want to <b>permanently delete</b> all " + commify(total) + " search results?", ['trash-can', 'Delete All'], function(result) {
			if (!result) return;
			app.clearError();
			Dialog.showProgress( 1.0, "Starting Bulk Delete..." );
			
			app.api.post( 'app/bulk_search_delete', { index: 'alerts', query: query || '*' }, function(resp) {
				Dialog.hideProgress();
