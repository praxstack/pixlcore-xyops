...
class Maintenance {
	...
	expireServerCache() {
		// expire old servers out of serverCache
		var now = Tools.timeNow(true);
		var max_sec = Tools.getSecondsFromText( this.config.get('server_cache_max_time') || '24 hours' );
		var pruned = false;
		
		for (var key in this.serverCache) {
			if (this.serverCache[key].modified < now - max_sec) {
				delete this.serverCache[key];
				pruned = true;
			}
		}
		
		if (pruned) this.doUserBroadcastAllServers(true);
	}
	...
	dbSearchDelete(opts) {
		...
		// optionally write deleted rows to a backup file (XYBK)
		if (opts.backup) {
			// allow for date/time placeholders in file path
			opts.backup = Tools.formatDate( Tools.timeNow(true), opts.backup );
			
			// print XYBK header
			try { 
				Tools.mkdirp.sync( Path.dirname(opts.backup) );
				fs.appendFileSync( opts.backup, 
					(fs.existsSync(opts.backup) ? "\n" : "") + 
					"# xyOps Data Export v1.0\n" + 
					"# Hostname: " + os.hostname() + "\n" + 
					"# Date/Time: " + (new Date()).toString() + "\n" + 
					"# Format: NDJSON\n" + 
					"\n# Database Index: " + opts.index + " (" + opts.query + ")\n" );
			}
			catch (err) {
				this.logError('fs', "Failed to write to backup log: " + opts.backup + ": " + err);
				if (opts.callback) opts.callback(err);
				return;
			}
			opts.ids = false; // full load mode
		...
		async.whilst(
							...
							var finish = function() {
								...
								self.unbase.delete( opts.index, id, function(err) {
									if (err) {
										...
										if (opts.backup) try { fs.appendFileSync( opts.backup, JSON.stringify({ index: opts.index, id: id, record: record }) + "\n" ); }
										catch (err) { self.logError('fs', "Failed to write to backup log: " + opts.backup + ": " + err); }
									}
									setTimeout( function() { callback(); }, opts.sleep );
								} ); // unbase.delete
							}; // finish
							
							if (opts.iterator) opts.iterator(record, opts, finish);
							else finish();
						},
						function(err) {
							if (err) {
			...
			function(err) {
				...
				if (opts.backup) try { fs.appendFileSync( opts.backup, "\n# End of file\n" ); }
				catch (err) { self.logError('fs', "Failed to write to backup log: " + opts.backup + ": " + err); }
	...
	dbSearchDelete_jobs(job, opts, callback) {
				...
				async.eachSeries( job_ids, function(job_id, callback) {
					self.unbase.get( 'jobs', job_id, function(err, sub_job) {
						...
						self.unbase.delete( 'jobs', job_id, function(err) {
							...
							// append sub-job to backup log if enabled
							if (opts.backup) try { fs.appendFileSync( opts.backup, JSON.stringify({ index: 'jobs', id: job_id, record: sub_job }) + "\n" ); }
							catch (err) { self.logError('fs', "Failed to write to backup log: " + opts.backup + ": " + err); }
							...
							self.dbSearchDelete_jobs(sub_job, opts, callback);
						} ); // unbase.delete
					} ); // unbase.get
				}, callback); // eachSeries
			}
	...
	dbSearchDelete_servers(server, opts, callback) {
