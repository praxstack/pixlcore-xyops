...
class Buckets {
	...
	api_get_bucket(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (!bucket) return self.doError('notfound', "Bucket not found: " + params.id, callback);
			
			var bucket_path = 'buckets/' + bucket.id;
			
			self.storage.head( bucket_path + '/data', function(err, meta) {
				if (err) self.doError('bucket', "Failed to locate bucket data: " + params.id, callback);
				...
				self.storage.getMulti( [ bucket_path + '/data', bucket_path + '/files' ], function(err, values) {
					...
					callback({ code: 0, bucket, data, meta, files });
				} ); // storage.getMulti
			}); // storage.head
		} ); // loaded session
	}
	...
	api_write_bucket_data(args, callback) {
		...
		// note: data is shallow-merged unless 'replace' param is set
		var self = this;
		var params = Tools.mergeHashes( args.params, args.query );
		if (!this.requireMaster(args, callback)) return;
		...
		this.loadSession(args, function(err, session, user) {
			...
			async.series([
				...
				function(callback) {
					// load bucket data (skip if replacing)
					if (params.replace) return process.nextTick(callback);
				...
				function(callback) {
					...
					if (params.replace) bucket_data = params.data || {};
					else Tools.mergeHashInto( bucket_data, params.data || {} );
	...
	api_empty_bucket(args, callback) {
		// delete all files and/or data from storage bucket
		var self = this;
		var params = Tools.mergeHashes( args.params, args.query );
		...
		if (!params.files && !params.data) {
			return self.doError('bucket', "Please specify files and/or data to empty from bucket.", callback);
		}
		...
		this.loadSession(args, function(err, session, user) {
			...
			async.series([
				...
				function(callback) {
					// load bucket files
					if (!params.files) return process.nextTick(callback);
					self.storage.get( bucket_path + '/files', function(err, files) {
						if (err) return callback(err);
						bucket_files = files;
						callback();
				...
				function(callback) {
					// write bucket file metadata
					if (!params.files) return process.nextTick(callback);
					self.storage.put( bucket_path + '/files', [], callback );
				},
				function(callback) {
					// write bucket data
					if (!params.data) return process.nextTick(callback);
					self.storage.put( bucket_path + '/data', {}, callback );
				}
			],
			function(err) {
				self.storage.unlock( bucket_path );
				if (err) {
					return self.doError('bucket', "Failed to empty bucket: " + (err.message || err), callback);
				}
				callback({ code: 0 });
			}); // async.series
		} ); // loaded session
