...
class Plugins {
	...
	api_update_plugin(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			// do not allow plugin type to be changed
			delete params.type;
	...
	requireValidPluginData(params, callback) {
		...
		if (params.params) {
			...
			for (var idx = 0, len = params.params.length; idx < len; idx++) {
				...
				// Value-bearing parameter types use the same schema as toolset fields.
				// Other types, such as group, bucket, system and toolset, intentionally
				// store their configuration in different properties and have no value.
				if (param.type.match(/^(checkbox|code|hidden|json|select|text|textarea)$/)) {
					if (!('value' in param)) return this.doError('api', err_prefix + ": Value property is missing", callback);
					
					if (param.type == 'checkbox') {
						if ((typeof(param.value) != 'boolean') && (param.value !== 1) && (param.value !== 0)) {
							return this.doError('api', err_prefix + ": Checkbox value must be a boolean", callback);
						}
					}
					else if (param.type == 'json') {
						if (typeof(param.value) != 'object') return this.doError('api', err_prefix + ": JSON value must be an object", callback);
					}
					else if ((param.type == 'text') && (param.variant == 'number')) {
						if ((typeof(param.value) != 'number') && (param.value !== null)) {
							return this.doError('api', err_prefix + ": Numeric value must be a number or null", callback);
						}
					}
					else if (typeof(param.value) != 'string') {
						return this.doError('api', err_prefix + ": Value must be a string", callback);
					}
				}
