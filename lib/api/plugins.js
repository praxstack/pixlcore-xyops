...
class Plugins {
	...
	requireValidPluginData(params, callback) {
		...
		if (params.params) {
			...
			for (var idx = 0, len = params.params.length; idx < len; idx++) {
				...
				if (param.id.match(/^_xy_override_(plugin|params|id|event|enabled|category|fields|actions|limits|triggers|workflow)$/)) {
	...
	requireValidToolsetData(param, params, callback) {
		...
		data.tools.forEach( function(tool, idx) {
			...
			if (tool.id.match(/^_xy_override_(plugin|params|id|event|enabled|category|fields|actions|limits|triggers|workflow)$/)) {
