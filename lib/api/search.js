...
class Search {
	...
	api_search_jobs(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			if (!privs.admin) {
				...
				if (cats.length || cgrps.length) params.query = String(params.query).replace(/^\s*\#id\:\S+/i, '');
			} // non-admin
			
			if (!params.query) params.query = '*';
			params.username = user.username || user.id;
			
			self.unbaseSearch( 'jobs', params.query, params, function(err, results) {
	...
	api_search_servers(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			if (!privs.admin) {
				...
				// if user is limited, prevent `#id:` unbase shortcut
				if (cgrps.length) params.query = String(params.query).replace(/^\s*\#id\:\S+/i, '');
			} // non-admin
			
			if (!params.query) params.query = '*';
			params.username = user.username || user.id;
			
			self.unbaseSearch( 'servers', params.query, params, function(err, results) {
				if (err) return self.doError('db', "Failed DB search: " + err, callback);
				if (!results.records) results.records = [];
	...
	api_search_alerts(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.unbaseSearch( 'alerts', params.query, params, function(err, results) {
				if (err) return self.doError('db', "Failed DB search: " + err, callback);
				if (!results.records) results.records = [];
	...
	api_search_snapshots(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.unbaseSearch( 'snapshots', params.query, params, function(err, results) {
				if (err) return self.doError('db', "Failed DB search: " + err, callback);
				if (!results.records) results.records = [];
	...
	api_search_revision_history(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			// if user is category or group limited, return empty array (non-error, as to not break the UI)
			var cuser = self.getComputedUser(user);
			if (cuser.categories.length || cuser.groups.length) return callback({ code: 0, rows: [], list: { length: 0 } });
	...
	api_bulk_search_export(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			if (err) return self.doError('session', err.message, callback);
			if (!self.requireValidUser(session, user, callback)) return;
			if (!self.requireUnrestrictedUser(user, callback)) return;
	...
	ws_search_job_files(args) {
		...
		var privs = socket.computed.privileges;
		if (!privs.admin) {
			// if user has limited category access, augment search query accordingly
			var cats = socket.computed.categories;
			if (cats.length) {
				if (!params.query) params.query = '';
				params.query += ' category:' + cats.join('|');
			}
			...
			// if user has limited group access, augment search query accordingly
			var cgrps = socket.computed.groups;
			if (cgrps.length) {
				if (!params.query) params.query = '';
				params.query += ' groups:' + cgrps.join('|');
			}
