...
class Configuration {
	...
	api_get_multiple(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (params.jobs) {
				resp.activeJobs = self.getActiveJobs();
				resp.internalJobs = self.internalJobs;
				resp.jobRateLimits = self.jobRateLimits;
			}
			if (params.alerts) {
				resp.activeAlerts = self.activeAlerts;
			}
			if (params.state) {
				resp.state = Tools.copyHashRemoveKeys( self.state, { dirty: 1 } );
			}
			if (params.stats) {
				resp.stats = self.stats;
			}
			if (params.servers) {
