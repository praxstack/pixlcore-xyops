...
class Admin {
	...
	formatOptimizationReport(report) {
		// Build a human-friendly Markdown report, then include the raw JSON.
		function formatInline(value) {
			// Keep arbitrary engine values safe and readable in Markdown bullets.
			if ((value === null) || (typeof(value) == 'undefined')) return '';
			if (typeof(value) == 'number') return Tools.commify(value);
			if (typeof(value) == 'boolean') return value ? 'Yes' : 'No';
			return '`' + ('' + value).replace(/`/g, "'") + '`';
		}
		
		function getReportItems(report) {
			// Hybrid storage returns nested engine reports.  Most display sections
			// should use the leaf engine reports so operations are not duplicated.
			if (report && Array.isArray(report.engines)) {
				return report.engines.reduce( function(list, item) {
					return list.concat( getReportItems(item) );
				}, [] );
			}
			return report ? [ report ] : [];
		}
		
		function getOperations(report, name) {
			// Pull matching operations from leaf reports and carry parent context.
			var operations = [];
			getReportItems(report).forEach( function(item) {
		...
		function appendEngineContext(lines, item) {
			// Add only the context fields that apply to the current engine.
			if (item.role) lines.push("- **Role:** " + formatInline(item.role));
			if (item.engine) lines.push("- **Engine:** " + formatInline(item.engine));
			if (item.file) lines.push("- **Database File:** " + formatInline(item.file));
			if (item.table) lines.push("- **Database Table:** " + formatInline(item.table));
		}
		
		function appendPerf(lines, title, report, level) {
			// The pixl-perf report stores timings under perf.perf.
			var perf = report && report.perf && report.perf.perf;
			var keys = perf ? Object.keys(perf) : [];
			if (!keys.length) return;
			
			lines.push("\n" + (level || "####") + " " + title + "\n");
			keys.forEach( function(key) {
				lines.push("- **" + key.replace(/_/g, ' ') + ":** " + Tools.getTextFromSeconds(perf[key], true, false));
			} );
		}
		
		var lines = [];
		var items = getReportItems(report);
		var vacuum_ops = getOperations(report, 'vacuum');
		var checkpoint_ops = getOperations(report, 'wal_checkpoint');
		var integrity_ops = getOperations(report, 'integrity_check');
		
		lines.push("### Database Optimization Report\n");
		if (report && report.optimized) lines.push("The storage optimization job completed successfully.");
		else lines.push("The storage optimization job completed successfully. No active optimization was required for this storage engine.");
		
		lines.push("\n#### Summary\n");
		appendEngineContext(lines, report || {});
		lines.push("- **Optimization Performed:** " + (report && report.optimized ? 'Yes' : 'No'));
		
		if (items.length > 1) {
			lines.push("\n#### Storage Engines\n");
			items.forEach( function(item) {
				var label = [];
				if (item.role) label.push(item.role);
				if (item.engine) label.push(item.engine);
				lines.push("- **" + (label.join(' / ') || 'Engine') + ":** " + (item.optimized ? 'Optimized' : 'No optimization needed'));
			} );
		}
		
		if (vacuum_ops.length) {
			lines.push("\n#### Vacuum\n");
			lines.push("A database vacuum was included in this optimization run.");
			
			vacuum_ops.forEach( function(entry) {
				var item = entry.report;
				var operation = entry.operation;
				
				if (vacuum_ops.length > 1) {
					lines.push("\n**" + [ item.role, item.engine ].filter(Boolean).join(' / ') + "**");
				}
				
				appendEngineContext(lines, item);
				if (typeof(operation.ok) != 'undefined') lines.push("- **Status:** " + (operation.ok ? 'Completed' : 'Failed'));
				if (operation.command) lines.push("- **Command:** " + formatInline(operation.command));
				if (typeof(operation.rowCount) != 'undefined') lines.push("- **Rows Reported:** " + Tools.commify(operation.rowCount));
				if (typeof(operation.size_before) != 'undefined') lines.push("- **Size Before:** " + Tools.getTextFromBytes(operation.size_before) + " (" + Tools.commify(operation.size_before) + " bytes)");
				if (typeof(operation.size_after) != 'undefined') lines.push("- **Size After:** " + Tools.getTextFromBytes(operation.size_after) + " (" + Tools.commify(operation.size_after) + " bytes)");
				if (typeof(operation.bytes_reclaimed) != 'undefined') lines.push("- **Bytes Reclaimed:** " + Tools.getTextFromBytes(operation.bytes_reclaimed) + " (" + Tools.commify(operation.bytes_reclaimed) + " bytes)");
				if (operation.reclaimed && (typeof(operation.bytes_reclaimed) == 'undefined')) lines.push("- **Space Reclaimed:** " + formatInline(operation.reclaimed));
			} );
		}
		
		if (checkpoint_ops.length) {
			lines.push("\n#### WAL Checkpoint\n");
			lines.push("SQLite WAL checkpoint details are shown below.");
			
			checkpoint_ops.forEach( function(entry) {
				var operation = entry.operation;
				appendEngineContext(lines, entry.report);
				if (typeof(operation.ok) != 'undefined') lines.push("- **Status:** " + (operation.ok ? 'Completed' : 'Needs Attention'));
				if (operation.mode) lines.push("- **Mode:** " + formatInline(operation.mode));
				if (operation.journal_mode) lines.push("- **Journal Mode:** " + formatInline(operation.journal_mode));
				if (typeof(operation.busy) != 'undefined') lines.push("- **Busy Frames:** " + Tools.commify(operation.busy));
				if (typeof(operation.log) != 'undefined') lines.push("- **Log Frames:** " + Tools.commify(operation.log));
				if (typeof(operation.checkpointed) != 'undefined') lines.push("- **Checkpointed Frames:** " + Tools.commify(operation.checkpointed));
			} );
		}
		
		if (integrity_ops.length) {
			lines.push("\n#### Integrity Check\n");
			
			integrity_ops.forEach( function(entry) {
				var result = entry.operation.result || entry.report.integrity_check || '';
				appendEngineContext(lines, entry.report);
				lines.push("- **Status:** " + (entry.operation.ok ? 'Passed' : 'Failed'));
				if (result) {
					if (result.match(/\n/)) lines.push("- **Result:**\n\n```text\n" + result + "\n```");
					else lines.push("- **Result:** " + formatInline(result));
				}
			} );
		}
		
		var noop_ops = getOperations(report, 'noop');
		if (noop_ops.length && !vacuum_ops.length) {
			lines.push("\n#### Optimization Notes\n");
			noop_ops.forEach( function(entry) {
				var message = entry.operation.message || "No optimization was required for this engine.";
				lines.push("- " + message);
			} );
		}
		
		var perf_lines = [];
		appendPerf(perf_lines, "Overall", report || {}, "#####");
		items.forEach( function(item) {
			if (item === report) return;
			appendPerf(perf_lines, [ item.role, item.engine ].filter(Boolean).join(' / ') || 'Engine', item, "#####");
		} );
		if (perf_lines.length) {
			lines.push("\n#### Performance Metrics");
			lines = lines.concat(perf_lines);
		}
	...
	api_get_servers(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			callback({
				code: 0,
				servers: self.getUserLimitedActiveServers( self.getComputedUser(user) ),
	...
	api_update_global_state(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			if (err) return self.doError('session', err.message, callback);
			if (!self.requireValidUser(session, user, callback)) return;
			if (!self.requirePrivilege(user, 'update_state', callback)) return;
	...
	api_bulk_search_delete_jobs(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (cuser.categories.length || cuser.groups.length) return self.doError('access', "Category or group limited users may not use this API", callback);
			...
			self.dbSearchDelete({
				...
				callback: function() {
					// if any users are sitting on search pages, refresh them
					self.doPageBroadcast('Search', 'bulk_delete_completed', {});
				}
			});
	...
	api_bulk_search_delete_tickets(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.dbSearchDelete({
				...
				username: user.username,
				callback: function() {
					// if any users are sitting on ticket (search) pages, refresh them
					self.doPageBroadcast('Tickets', 'bulk_delete_completed', {});
				}
			});
	...
	api_bulk_search_delete(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var job = self.dbSearchDelete({
				...
				username: user.username,
				callback: function() {
					// if any users are sitting on applicable pages, trigger a refresh
					switch (params.index) {
						case 'jobs': self.doPageBroadcast('Search', 'bulk_delete_completed', {}); break;
						case 'tickets': self.doPageBroadcast('Tickets', 'bulk_delete_completed', {}); break;
						case 'alerts': self.doPageBroadcast('Alerts', 'bulk_delete_completed', {}); break;
						case 'snapshots': self.doPageBroadcast('Snapshots', 'bulk_delete_completed', {}); break;
					}
				}
			});
	...
	api_admin_run_optimization(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.optimize( function(err, report) {
				if (err) {
					self.logError('db', "DB Optimization Error: " + err);
					job.details += "\n**Database Optimization Error:** " + err + "\n";
					job.finish();
					return;
				}
				
				job.details += self.formatOptimizationReport(report);
	...
	api_admin_reset_daily_stats(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (self.findInternalJobs({ type: 'db' }).length) {
				return self.doError('admin', "Another internal database job is already running.", callback);
			}
			if (self.findActiveJobs({}).length) {
				return self.doError('admin', "Cannot perform a stat reset while jobs are running.", callback);
			}
			
			self.logDebug(5, "Manually resetting daily stats", params );
			...
			if (params.recount) {
				...
				self.dbSearchUpdate({
					index: 'jobs',
					query: `date:>=${midnight} tags:_last`, // skip retries
					offset: 0,
					sort_by: '_id',
					sort_dir: 1,
					title: "Custom user job stat recount",
					username: user.username,
					threads: 1,
					
					iterator: function(job, callback) {
						var conditions = ['start'].concat( self.getCompletedJobConditions(job) );
						self.incrementDailyJobStats(job, conditions);
						...
						callback(null, false); // do not update job
					}, // iterator
					
					callback: function(err) {
						// all done
						self.logDebug(5, "Bulk recount is complete");
						self.doUserBroadcastAll( 'update', { stats: self.stats } );
						
						if (disabled_scheduler) {
							// re-enable scheduler if we disabled it
							self.putState(state_key, true);
							self.logTransaction('state_update', state_key, self.getClientInfo(args, { key: state_key, value: true }));
						}
					} // callback
				}); // dbSearchUpdate
			}
	...
	api_admin_reset_job_rate_limits(args, callback) {
		// reset job rate limit windows
		var self = this;
		var params = args.params;
		if (!this.requireMaster(args, callback)) return;
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (params.id) {
				self.logDebug(4, "Resetting single rate limit window: " + params.id);
				delete self.jobRateLimits[ params.id ];
			}
			else {
				self.logDebug(4, "Resetting all rate limit windows");
				self.jobRateLimits = {};
			}
	...
	api_admin_import_data(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			async.series([
				...
				function(callback) {
					// secret metadata was reloaded above, so rebuild the encrypted data cache to match
					self.reloadSecretCache(callback);
				}
			], 
			function(err) {
				// all done, or errored out
				...
				if (err) {
					...
					// monitor workers cache secret-backed command data, so refresh it after import
					self.doServerBroadcastAll('update', {
						plugins: Tools.findObjects( self.plugins, { type: 'event' } ),
						commands: self.getCommandsWithSecrets()
					});
					
					// force all users to reconnect so they get updated data
					Object.values(self.sockets).forEach( socket => {
						if (socket.auth && socket.user) socket.close();
					} );
				}
			}); // series
		} ); // loaded session
	...
	api_admin_export_data(args, callback) {
		...
		if (params.lists && (typeof(params.lists) == 'string')) {
			if (params.lists === "all") params.lists = this.config.getPath('ui.list_list').map( function(item) { return item.id; } );
			else params.lists = [ params.lists ];
		}
		if (params.indexes && (typeof(params.indexes) == 'string')) {
			if (params.indexes === "all") params.indexes = this.config.getPath('ui.database_list').map( function(item) { return item.id; } );
			else params.indexes = [ params.indexes ];
		}
		if (params.extras && (typeof(params.extras) == 'string')) {
			if (params.extras === "all") params.extras = this.config.getPath('ui.extra_list').map( function(item) { return item.id; } );
			else params.extras = [ params.extras ];
		}
		...
		this.loadSession(args, function(err, session, user) {
			...
			var export_aborted = false;
			var stream = null;
			
			// Stop the background iterators before finishing their internal job.  Storage
			// callbacks may still arrive after the HTTP client has disconnected, and a
			// finished internal job no longer has its update() convenience method.
			function abortExport(message, err) {
				if (!callback) return;
				
				var request_callback = callback;
				callback = null;
				export_aborted = true;
				
				// Release the gzip stream and wake any pending write callbacks.  The outer
				// iterator checks export_aborted before attempting another export item.
				if (stream && !stream.destroyed) stream.destroy();
				
				job.finish();
				if (err) self.logError('export', message + err, params);
				else self.logError('export', message, params);
			...
			args.response.on('error', function(err) {
				abortExport("Data export response error: ", err);
			});
			args.response.on('close', function() {
				abortExport("Data export connection terminated");
			});
			...
			exportTypes.jobFiles = function(item, callback) {
				...
				if (!item.query) item.query = 'tags:_files';
				if (!item.max_size) item.max_size = 1024 * 1024;
			...
			async.eachSeries( params.items,
				function(item, callback) {
					// A response error or disconnect finishes the internal job immediately.
					// Stop here when the current export type eventually yields control.
					if (export_aborted) return callback(true);
				...
				function() {
					// The response handlers already cleaned everything up on an abort.
					if (export_aborted) return;
		...
		this.loadSession(args, function(err, session, user) {
			...
			async.eachSeries( server_ids,
				function(server_id, callback) {
					...
					// mark server as disabled during upgrade, so no new jobs get dispatched to it
					server.enabled = false;
					self.doUserBroadcastAllServers();
		...
		this.loadSession(args, function(err, session, user) {
			...
			async.eachSeries( host_ids,
				...
				function(err) {
					...
					if (!host_ids.length) stagger_sec = 5;
	...
	api_admin_broadcast_message(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.logDebug(5, "Broadcasting message to all users", params);
			self.doUserBroadcastAll('notify', params);
			
			callback({ code: 0 });
		}); // loadSession
	}
	
	api_admin_multi_update_all(args, callback) {
		// update all items in any global list (admin only)
		var self = this;
		var params = args.params;
		if (!this.requireMaster(args, callback)) return;
		
		if (!params.list || !Tools.findObject( this.config.getPath('ui.list_list'), { id: params.list } )) {
			return this.doError('api', "Request missing 'list' parameter, or it is invalid.", callback);
		}
		if (params.list == 'users') {
			return this.doError('api', "Cannot update users with this API.", callback);
		}
		if (params.list == 'secrets') {
			return this.doError('api', "Cannot update secrets with this API.", callback);
		}
		if (!params.update && !params.delete) {
			return this.doError('api', "Missing both 'update' and 'delete' parameters", callback);
		}
		if (params.update && !Tools.isaHash(params.update)) {
			return this.doError('api', "Invalid 'update' parameter (must be an object)", callback);
		}
		if (params.delete && !Tools.isaHash(params.delete)) {
			return this.doError('api', "Invalid 'delete' parameter (must be an object).", callback);
		}
		
		var list_name = params.list;
		var list_path = 'global/' + list_name;
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.logDebug(5, "Performing multi-update on: " + list_path, params);
			
			self.storage.listEachPageUpdate( list_path,
					...
					for (var idx = 0, len = items.length; idx < len; idx++) {
						...
						if (params.update) {
							for (var key in params.update) { 
								Tools.setPath( item, key, params.update[key] ); 
							}
						}
						if (params.delete) {
							for (var key in params.delete) { 
								Tools.deletePath( item, key ); 
							}
						}
					}
				...
				function(err) {
					...
					self.logDebug(5, "Successfully updated all " + list_name);
					self.logTransaction( list_name + '_multi_update', `Admin bulk update applied (all ${list_name})`, self.getClientInfo(args, params));
					
					callback({ code: 0 });
					
					// update cache in background
					self.storage.listGet( list_path, 0, 0, function(err, items) {
						if (err) {
							// this should never fail, as it should already be cached
							self.logError('storage', "Failed to cache list: " + list_path + ": " + err);
							return;
						}
						self[ list_name ] = items;
						
						// re-prep plugins (they may have changed)
						if (list_name == 'plugins') {
							self.prepPlugins();
						...
						else if ((list_name == 'alerts') || (list_name == 'monitors')) {
							self.precompileMonitors();
						}
