...
class Satellite {
	...
	getSatelliteBaseURL(request) {
		// Start with the web server's standard self-referencing URL.
		var base_url = this.web.getSelfURL(request).replace(/\/$/, '');
		
		// Preserve an explicit external port from the request.  Docker and reverse
		// proxies may forward it to a different internal WebServer port.
		if (request.headers.host.match(/:\d+$/)) {
			base_url = (request.headers.ssl ? 'https://' : 'http://') + request.headers.host;
		}
	...
	api_get_satellite_releases(args, callback) {
		...
		var sat = this.config.get('satellite');
		var cache_file = Path.join( this.config.get('temp_dir'), 'xysat-releases.json' );
		var cache_ttl = sat.cache_ttl || 3600;
		if (!this.requireMaster(args, callback)) return;
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (sat.disable_version_checks || (sat.enable_version_checks === false)) {
			...
			// use cached sat release data, or fetch from origin if stale
			fs.stat( cache_file, function(err, stats) {
				if (err || (stats.mtimeMs / 1000 < Tools.timeNow() - cache_ttl)) {
					...
					self.request.json( sat.list_url, false, { retries: 8, retryDelay: 50 }, function(err, resp, data, perf) {
						...
						if (!data || !Array.isArray(data) || !data.length) {
							return self.doError('multi', "Failed to fetch satellite release list: Invalid response from provider", callback);
						}
						...
						Tools.writeFileAtomic( cache_file, JSON.stringify(data), function(err) {
							if (err) return self.doError('satellite', "Failed to write cache file: " + cache_file + ": " + err, callback);
				...
				// use cached file
				self.logDebug(9, "Using satellite release cache: " + cache_file);
				
				fs.readFile( cache_file, 'utf8', function(err, contents) {
					if (err) return self.doError('satellite', "Failed to read cache file: " + cache_file + ": " + err, callback);
					var data = null;
					
					try { data = JSON.parse(contents); }
					catch (err) { return self.doError('satellite', "Failed to parse cache file: " + cache_file + ": " + err, callback); }
	...
	fetch_satellite_install(args, callback) {
		...
		fs.readFile( `internal/${filename}`, 'utf8', function(err, template) {
			...
			var script = Tools.sub( template, {
				...
				base_url: self.getSatelliteBaseURL(args.request)
			} );
