...
class APIKeyManagement {
	...
	api_get_api_key(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.listFind( 'global/api_keys', { id: params.id }, function(err, item) {
				if (err || !item) {
					return self.doError('notfound', "Failed to locate API Key: " + params.id, callback);
				}
	...
	api_create_api_key(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			// skip this if importing a key
			var plain_key = '';
			if (!params.key) {
				plain_key = Tools.generateUniqueBase64();
			...
			else if (!params.mask) params.mask = '********';
