...
class Monitors {
	...
	loadMonitorServer(server_id, callback) {
		// load a server record from memory or the database so monitor data
		// can be authorized before any host or timeline records are loaded
		var server = this.servers[server_id] || this.serverCache[server_id];
		if (server) return callback(null, server);
		this.unbase.get( 'servers', server_id, callback );
	}
	...
	api_create_monitor(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (grps.length) return self.doError('alert', "Group-limited users may not manage monitors.", callback);
	...
	api_get_quickmon_data(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var servers = self.getUserLimitedActiveServers(cuser);
			
			if (params.server) {
				// include single server
				if (self.servers[params.server] && !servers[params.server]) {
					return self.doError('access', "You do not have the required account privileges to access server ID " + params.server + ".", callback);
				}
				if (servers[params.server] || !cuser.groups.length) {
					resp.servers[ params.server ] = self.quickMonCache[ params.server ] || [];
				}
			}
			else if (params.group) {
				// include all online servers in specified group
				if (!self.requireGroupPrivilege(user, params.group, callback)) return;
				...
				// include all servers visible to this user
				for (var server_id in servers) {
					resp.servers[ server_id ] = self.quickMonCache[ server_id ] || [];
				}
			}
	...
	api_get_latest_monitor_data(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.loadMonitorServer(params.server, function(err, server) {
				if (err || !server) return self.doError('notfound', "Failed to locate server: " + params.server, callback);
