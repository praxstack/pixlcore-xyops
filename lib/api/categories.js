...
class Categories {
	...
	api_get_categories(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var cats = self.getUserLimitedCategories( self.getComputedUser(user) );
	...
	api_get_category(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var category = Tools.findObject( self.categories, { id: params.id } );
			if (!category) return self.doError('notfound', "Failed to locate category: " + params.id, callback);
			
			if (!self.requireCategoryPrivilege(user, category.id, callback)) return;
			
			// success, return item
			callback({ code: 0, category: category });
	...
	api_create_category(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.listPush( 'global/categories', params, function(err) {
				...
				self.categories.push( Tools.copyHash(params, true) );
				
				// send api response
				callback({ code: 0, category: params });
				
				// update all users
				self.doUserBroadcastAllCategories();
	...
	api_update_category(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.listFindUpdate( 'global/categories', { id: params.id }, params, function(err, category) {
				...
				Tools.mergeHashInto( Tools.findObject( self.categories, { id: params.id } ) || {}, category );
	...
	api_delete_category(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.listFindDelete( 'global/categories', { id: params.id }, function(err, category) {
				...
				Tools.deleteObject( self.categories, { id: params.id } );
