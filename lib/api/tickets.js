...
class Tickets {
		...
		this.loadSession(args, function (err, session, user) {
			...
			if (params.id) {
				self.unbase.get( 'tickets', params.id, function(err, record) {
					...
					if (!self.requireCategoryPrivilege(user, record.category, callback)) return;
					callback({ code: 0, ticket: record });
				}); // unbase.get
			}
			else if (params.num) {
				self.unbaseSearch( 'tickets', 'num:' + params.num, { offset:0, limit:1 }, function(err, results) {
					...
					if (!self.requireCategoryPrivilege(user, results.records[0].category, callback)) return;
		...
		this.loadSession(args, function(err, session, user) {
			...
			async.eachLimit( params.ids, self.storage.concurrency,
				...
				function() {
					...
					// Convert tickets to an array while preserving exactly one response
					// position per requested ID.  Missing and inaccessible tickets use
					// the same generic placeholder so resource existence is not disclosed.
					var cats = self.getComputedCategories(user);
					var final_tickets = params.ids.map( function(id) {
						var ticket = tickets[id];
						if (!ticket || ticket.err || (cats.length && !cats.includes(ticket.category))) {
							return { err: "Failed to locate ticket: " + id };
						}
						return ticket;
					} );
	...
	api_search_tickets(args, callback) {
		...
		var count = (params.count == 1) || (params.count == "true");
		if (count) params.ids = true;
		delete params.count;
		...
		this.loadSession(args, function (err, session, user) {
			...
			if (!privs.admin) {
				...
				// if user is limited, prevent `#id:` unbase shortcut
				if (cats.length) params.query = String(params.query).replace(/^\s*\#id\:\S+/i, '');
			} // non-admin
			
			if (!params.query) params.query = '*';
			params.username = user.username || user.id;
			
			if (count && (params.query in self.ticketQueryCountCache)) {
				// fresh cache hit, return quickly
				var total = self.ticketQueryCountCache[params.query];
				self.logDebug(9, "Ticket search count cache hit", { query: params.query, total: total });
				return callback({ code: 0, list: { length: total } });
			}
			
			self.unbaseSearch( 'tickets', params.query, params, function(err, results) {
				if (err) return self.doError('ticket', "Failed DB search: " + err, callback);
				
				if (compact && results.total && !count) {
				...
				if (count) {
					// count only, no ticket records
					results.records = null;
					self.ticketQueryCountCache[params.query] = results.total;
					self.logDebug(9, "Ticket search count cache miss", { query: params.query, total: results.total });
				}
		...
		this.loadSession(args, function(err, session, user) {
			...
			function(err) {
				...
				self.unbase.insert( { index: 'tickets', id: params.id, data: params, fast: true }, function(err) {
					...
					// notify all users that tickets have changed, and clear count cache
					self.doUserBroadcastAll('tickets_changed', {});
					self.ticketQueryCountCache = {};
	...
	api_update_ticket(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			function(err) {
				...
				if (!self.requireCategoryPrivilege(user, ticket.category, callback)) { self.storage.unlock( lock_key ); return; }
				if (('category' in params) && !self.requireCategoryPrivilege(user, params.category, callback)) { self.storage.unlock( lock_key ); return; }
				...
				self.unbase.update( { index: 'tickets', id: params.id, data: params, fast: true }, function(err, ticket) {
					...
					self.doPageRestrictedBroadcast( 'Tickets?id=' + ticket.id, 'ticket_updated', { ticket, username: user.username || user.id }, ticket );
					self.doPageRestrictedBroadcast( 'Tickets?num=' + ticket.num, 'ticket_updated', { ticket, username: user.username || user.id }, ticket );
			...
			async.eachSeries( Object.values(args.files),
				...
				function(err) {
					...
					self.unbase.update( 'tickets', params.ticket, function(ticket) {
						...
						if (!self.requireCategoryPrivilege(user, ticket.category, callback)) {
							return false; // false triggers 'ABORT' flow below
						}
	...
	api_delete_ticket(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.unbase.get( 'tickets', params.id, function(err, old) {
				...
				if (!self.requireCategoryPrivilege(user, old.category, callback)) return;
	...
	requireValidTicketParams(params, callback) {
		...
		if (params.category && !Tools.findObject(this.categories, { id: params.category })) {
			return this.doError('ticket', "Unknown category: " + params.category, callback);
		}
