...
class Groups {
	...
	api_get_groups(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var grps = self.getUserLimitedGroups( self.getComputedUser(user) );
			
			callback({
				code: 0,
				rows: grps,
				list: { length: grps.length }
			});
	...
	api_get_group(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (!self.requireGroupPrivilege(user, group.id, callback)) return;
	...
	api_create_group(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.listPush( 'global/groups', params, function(err) {
				...
				self.doUserBroadcastAllGroups(true);
