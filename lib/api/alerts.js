...
class Alerts {
	...
	api_create_alert(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (grps.length) return self.doError('alert', "Group-limited users may not manage alerts.", callback);
			...
			async.eachLimit( params.ids, self.storage.concurrency,
				function(id, callback) {
					self.unbase.get( 'alerts', id, function(err, alert) {
						// Preserve one response position per requested ID, but do not
						// distinguish a missing invocation from an inaccessible one.
						if (alert && self.getUserLimitedAlerts(cuser, [alert]).length) alerts[id] = alert;
						else alerts[id] = { err: "Failed to locate alert invocation: " + id };
						callback();
					}); // unbase.get
				},
				function(err) {
