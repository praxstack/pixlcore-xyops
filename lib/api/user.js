...
class Users {
	...
	api_user_settings(args, callback) {
		...
		// category and group restrictions are administrator-controlled, just like
		// privileges and roles, and must never be changed through user settings
		...
		this.usermgr.sanitizeUser(params);
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.put( path, user, function(err) {
				...
				var stub_user = { username: user.username, full_name: user.full_name, icon: user.icon, roles: user.roles };
				self.doUserBroadcastAll( 'single', { list: 'users', item: stub_user } );
				
				// and send redeacted user update to user's own sockets
				var redacted_user = Tools.copyHashRemoveKeys( user, { password: 1, salt: 1 } );
				self.doUserBroadcast( user.username, 'self_update', { user: redacted_user }) ;
