...
class WebHooks {
	...
	api_get_web_hook(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (!web_hook) return self.doError('notfound', "Failed to locate web hook: " + params.id, callback);
	...
	api_create_web_hook(args, callback) {
		...
		if (params.headers && !Array.isArray(params.headers)) {
			return this.doError('web_hook', "Invalid header format for web hook: " + params.id, callback);
		}
		
		if (!('enabled' in params)) params.enabled = true;
		if (!params.body) params.body = '';
		if (!params.timeout) params.timeout = 30;
		if (!params.retries) params.retries = 0;
		if (!params.follow) params.follow = false;
		if (!params.ssl_cert_bypass) params.ssl_cert_bypass = false;
		if (!params.max_per_day) params.max_per_day = 0;
	...
	api_test_web_hook(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var hook_data = {
				_fallback: "Web Hook Test: " + combo_hook.title // macro substitution fallback value
			};
			...
			self.fireWebHook(combo_hook, hook_data, function(err, result) {
				if (err) {
					return callback({ code: 'webhook', description: '' + err });
				}
	...
	api_fire_job_web_hook(args, callback) {
		// fire web hook in context of a running job
		// auth is combination of job id (must be active), server id (must be active), and hook id (must be valid)
		var self = this;
		var params = args.params;
		if (!this.requireMaster(args, callback)) return;
		...
		// validate all three IDs, and return deliberately generic error messages on failure
		var job = this.activeJobs[ params.job ];
		if (!job) return this.doError('api', "Invalid.", callback);
		
		var server = this.servers[ params.server ];
		if (!server) return this.doError('api', "Invalid.", callback);
		
		var web_hook = Tools.findObject( this.web_hooks, { id: params.web_hook } );
		if (!web_hook) return this.doError('api', "Invalid.", callback);
		
		// {"enabled":true,"type":"web_hook","condition":"success","web_hook":"example_hook","text":"Custom: {{ event(job.event) }}"}
		var action = {
			enabled: true,
			type: 'web_hook',
			condition: 'continue',
		...
		this.runJobAction(job, action, function() {
			callback(action);
		}); // runJobAction
	}
