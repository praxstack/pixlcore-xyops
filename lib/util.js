...
class Util {
	...
	jexlSetup() {
		...
		jexl.addFunction('total', function(coll, key) {
			// add up all items in an array, or a specfic sub-key in the array items
			var total = 0;
			coll.forEach( item => total += key ? Tools.getPath(item, key) : item );
			return total;
		});
		
		jexl.addFunction('average', function(coll, key) {
			// average all items in an array, or a specfic sub-key in the array items
			var total = 0;
			coll.forEach( item => total += key ? Tools.getPath(item, key) : item );
			return total / (coll.length || 1);
		});
		
		jexl.addFunction('find', function(coll, key, value) {
			// find object in array by substring match in given key
			return coll.filter( function(item) { return !!(''+Tools.getPath(item, key)).includes(value); } );
		});
		...
		jexl.addFunction('match', function(str, pat) { 
			// support String.match
			if ((str === undefined) || (str === null)) return false;
			return !!String(str).match(pat); 
		});
		
		jexl.addFunction('server', function(id) {
			// convert server ID to custom label or hostname (online servers only)
			if (self.servers[id]) return self.servers[id].title || self.servers[id].hostname;
			else if (self.serverCache[id]) return self.serverCache[id].title || self.serverCache[id].hostname;
			else return id;
		});
		
		jexl.addFunction('event', function(id) {
			// convert event ID or job ID into event title
			var event = Tools.findObject(self.events, { id });
			if (event) {
				return event.title;
			}
			else if (self.activeJobs[id] && self.activeJobs[id].event) {
				event = Tools.findObject(self.events, { id: self.activeJobs[id].event });
	...
	stringValue(thing) {
		// coerce thing into string, but null/undefined become empty
		if (typeof(thing) == 'undefined') thing = '';
		else if (thing === null) thing = '';
		return '' + thing;
	}
	
	checkNumberRange(value, range) {
		// check number value vs. range string (MIN-MAX/STEP)
		value = parseFloat(value);
		
		var matches = String(range).replace(/\s+/g, '').match(/^(-?\d*\.?\d+)-(-?\d*\.?\d+)\/(any|-?\d*\.?\d+)$/i);
		...
		var step_num = parseFloat(step);
		
		// avoid floating point precision issues
		const epsilon = 0.0000001;
		const offset = (value - min) / step_num;
		
		return Math.abs(offset - Math.round(offset)) < epsilon;
	}
	...
	getLocalBaseURL() {
		// get base app url using localhost and web server port
		var hostname = this.config.get('local_api_hostname') || 'localhost';
		return `http://${hostname}:` + this.web.config.get('port');
	}
	...
	updateConfigOverrides(overrides, callback) {
		...
		// Data Migration: xyOps v1.0.90
		// Remove legacy `db_maint.` key prefixes (replaced with single top-level `db_maint` key).
		if (overrides.db_maint) Object.keys(this.server.configOverrides).filter( key => !!key.match(/^db_maint\.\S+$/) ).forEach( key => {
			delete self.server.configOverrides[key];
		} );
