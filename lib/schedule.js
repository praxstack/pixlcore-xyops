	...
	schedulerMinuteTick(dargs) {
		...
		this.events.forEach( function(event) {
			...
			while (cursor < now) {
				...
				schedules.forEach( function(trigger) {
					// Always compare against the cursor, as it may be replaying a missed minute.
					if ((trigger.type == 'single') && (trigger.epoch == cursor)) {
						scheduled = true;
						chosen_triggers.push(trigger);
						return;
					}
					if (trigger.type == 'interval') {
						// Calculate interval hits for this cursor minute, including during catch-up.
						// Piggy-back on the precision system to launch multiple jobs per minute.
						var hits = self.intervalHitsPerMinute(trigger, cursor);
						if (hits.length) {
							scheduled = true;
							chosen_triggers.push(trigger);
