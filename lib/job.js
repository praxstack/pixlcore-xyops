...
class Jobs {
	...
	launchJob(job, callback) {
		...
		var jlimit = Tools.findObject(job.limits, { type: 'job', enabled: true });
		if (qlimit && qlimit.amount && !jlimit) {
			return callback( new Error("The 'Max Queue Limit' requires a 'Max Jobs Limit' to also be present." ) );
		}
		
		// promote max job limit's cap key to top-level job prop (for performance)
		if (jlimit && jlimit.cap_key) job.cap_key = jlimit.cap_key;
		else delete job.cap_key;
		...
		if (plugin) {
			...
			this.applyPluginParamDefaults(job, plugin);
		...
		// if job has a cap_key, make sure all similar active jobs have the same concurrency limit set
		if (jlimit && job.cap_key) {
			this.appendMetaLog(job, `Shared capacity key: {${job.cap_key}}`);
			
			var jset = new Set();
			var qset = new Set();
			var rset = new Set();
			
			this.findSimilarJobs(job).forEach( function(temp_job) {
				var temp_jlimit = Tools.findObject( temp_job.limits || [], { type: 'job', enabled: true } );
				if (temp_jlimit && temp_jlimit.amount && temp_jlimit.cap_key) jset.add( temp_jlimit.amount );
				
				rset.add( (temp_jlimit && temp_jlimit.rate && temp_jlimit.window) ?
					`${temp_jlimit.rate}/${temp_jlimit.window}` :
					'none'
				);
				
				var temp_qlimit = Tools.findObject( temp_job.limits || [], { type: 'queue', enabled: true } );
				if (temp_qlimit && temp_qlimit.amount) qset.add( temp_qlimit.amount );
			} );
			
			if (jset.size > 1) this.appendMetaLog(job, `WARNING: Shared capacity key {${job.cap_key}} is shared amongst jobs with differing concurrency limits!`);
			if (qset.size > 1) this.appendMetaLog(job, `WARNING: Shared capacity key {${job.cap_key}} is shared amongst jobs with differing max queue limits!`);
			if (rset.size > 1) this.appendMetaLog(job, `WARNING: Shared capacity key {${job.cap_key}} is shared amongst jobs with differing rate limits!`);
		} // cap key
	...
	applyPluginParamDefaults(job, plugin) {
		// Plugins may gain new parameters after an Event has already been saved.
		// Fill in only missing values here at launch time, so existing Events do
		// not need to be updated whenever their Plugin definition changes.
		if (!job.params) job.params = {};
		(plugin.params || []).forEach( function(param) {
			// A missing value means the parameter has no default.  This can occur in
			// legacy data, so leave the job parameter absent instead of inventing one.
			if ((param.id in job.params) || !('value' in param)) return;
			
			if (param.type == 'select') {
				// Select defaults are encoded as the first item in a comma-separated
				// menu definition.  Invalid legacy values are skipped defensively.  New
				// malformed definitions are rejected by requireValidPluginData().
				if (typeof(param.value) != 'string') return;
				job.params[ param.id ] = param.value.replace(/\,.*$/, '').replace(/^.+\[([\w\-\.]+)\]\s*$/, '$1');
			}
			else {
				job.params[ param.id ] = (param.value && (typeof(param.value) == 'object')) ? Tools.copyHash(param.value, true) : param.value;
			}
		} );
	}
	...
	appendJobLog(job, msg, data) {
		...
		this.doPageRestrictedBroadcast( 'Job?id=' + job.id, 'log_append', { text: msg }, job );
		
		// add raw output to selected job streams as well (SSE)
		for (var req_id in this.web.requests) {
			var args = this.web.requests[req_id];
			if (args._xy_stream_job_output === job.id) {
				args.response.write("event: output\n");
				args.response.write("data: " + JSON.stringify({ xy: 1, type: 'output', text: msg }) + "\n\n");
			}
		}
	}
	...
	appendMetaLog(job, msg, data) {
		...
		this.doPageRestrictedBroadcast( 'Job?id=' + job.id, 'meta_row', row, job );
		
		// add meta output to selected job streams as well (SSE)
		for (var req_id in this.web.requests) {
			var args = this.web.requests[req_id];
			if (args._xy_stream_job_meta === job.id) {
				args.response.write("event: meta\n");
				args.response.write("data: " + JSON.stringify({ xy: 1, type: 'meta', row: row }) + "\n\n");
			}
		}
	}
	
	expireJobRateLimits() {
		// remove job rate limits if their windows have completed
		// called every tick, and when becoming master
		var now = Tools.timeNow(true);
		
		for (var queue_id in this.jobRateLimits) {
			var rate = this.jobRateLimits[queue_id];
			if (now >= rate.expires) delete this.jobRateLimits[queue_id];
		}
	}
	
	getJobQueueID(job) {
		// build up a job queue id using same logic as findSimilarJobs
		var queue_id = job.cap_key ? `cap:${job.cap_key}` : '';
		if (!queue_id) {
			var parts = [ job.event ];
	...
	monitorJobs() {
		...
		Object.values(this.activeJobs).forEach( function(job) {
			...
			if (job.state == 'queued') {
				var queue_id = self.getJobQueueID(job);
		...
		for (var queue_id in queues) {
			...
			// sort queue by priority, then by oldest
			...
			if (job_limit && job_limit.rate && job_limit.window) {
				if (!this.jobRateLimits[queue_id]) {
					// start fresh window
					this.jobRateLimits[queue_id] = { 
						id: queue_id, 
						count: 0, 
						max: job_limit.rate, 
						window: job_limit.window, 
						expires: this.getRateWindowExpiration(job_limit)
					};
				}
			}
			var rate = this.jobRateLimits[queue_id] || { count: 0, max: 9999999 };
			
			// reset counter if window passed
			if (job_limit && rate.expires && (now >= rate.expires)) {
				rate.count = 0;
				rate.expires = this.getRateWindowExpiration(job_limit);
				rate.max = job_limit.rate;
				rate.window = job_limit.window;
			}
			
			while (job_limit && (jobs.length < job_limit.amount) && (rate.count < rate.max)) {
				// we now have room!  launch queued job waiting the longest (also priority ones first)
				job = queued.shift();
				
				if (job && this.checkAvailableJobServer(job)) {
					this.appendMetaLog(job, "Moving job state from {queued} to {ready}");
					job.state = 'ready';
					delete job.position;
					this.monitorJob(job);
					
					// add current job to jobs array, for next while loop iteration
					jobs.push(job);
					
					// NOTE: No need to increment rate counter here, as monitorJob will do it
					...
					// no available server or last job processed -- break out of while loop
					job_limit = null;
				}
			} // while we have room
		} // foreach queue
	...
	monitorJob(job) {
		...
		switch (job.state) {
			...
			case 'ready':
				...
				// increment rate counter (if applicable)
				var queue_id = this.getJobQueueID(job);
				if (this.jobRateLimits[queue_id]) this.jobRateLimits[queue_id].count++;
			...
			case 'finishing':
				// the remote child has exited, but xySat may still be uploading logs / files
				// only check for a stale final update here (active runtime limits no longer apply)
				this.checkDeadJobTimeout(job);
			break;
		} // switch job.state
	}
	...
	checkDeadJobTimeout(job) {
		// check for stale job updates (used while both running and finishing)
		var now = Tools.timeNow();
		var timeout = this.config.get('dead_job_timeout');
		if (now - job.updated < timeout) return false;
		
		// job has timed out (no updates, server must be dead)
		job.retry_ok = true; // allow retry even though we're aborting
		this.abortJob(job, "No updates received in last " + Tools.getTextFromSeconds(timeout) + ", assuming job is dead.");
		return true;
	}
	...
	checkJobActiveLimits(job) {
		...
		if (this.checkDeadJobTimeout(job)) return;
	...
	triggerActiveJobLimit(job, limit) {
		...
		// set flag in job for adding tag later
		job.limited = true;
		...
		async.parallel(
			...
			function() {
				...
				self.doPageRestrictedBroadcast( 'Job?id=' + job.id, 'limit_triggered', { msg, limits: job.limits }, job );
	...
	getRateWindowExpiration(limit) {
		// calculate next expiration date for rate window
		var now = Tools.timeNow(true);
		var expires = 0;
		...
		switch (limit.window) {
			...
			case 60:
				expires = Tools.normalizeTime( now + 60, { sec: 0 } );
			break;
			
			case 3600:
				expires = Tools.normalizeTime( now + 3600, { sec: 0, min: 0 } );
			break;
			
			case 86400:
				var noon = Tools.normalizeTime( now, { sec: 0, min: 0, hour: 12 } );
				expires = Tools.normalizeTime( noon + 86400, { sec: 0, min: 0, hour: 0 } );
			break;
		}
		
		if (expires <= now) expires += limit.window; // DST nonsense
		return expires;
	}
	...
	checkJobStartLimits(job) {
			...
			if (jobs.length >= job_limit.amount) {
				...
				if (queue_limit && queue_limit.amount) {
					...
					if (queued.length < queue_limit.amount) {
						// room in queue, yay!
						this.appendMetaLog(job, "Moving job state from {" + job.state + "} to {queued} (max concurrent limit)");
						job.state = 'queued';
					...
					// no can run
					this.appendMetaLog(job, "Active jobs are blocking: " + jobs.map( job => '#' + job.id ).join(', ') );
					this.abortJob(job, "Maximum number of concurrent jobs for event has been reached, and queuing is disabled.");
					return false;
				}
			} // limit reached
		} // have limit
		...
		if (job_limit && job_limit.rate && job_limit.window) {
			...
			if (rate.count >= rate.max) {
				// rate limit reached -- can we queue?
				if (queue_limit && queue_limit.amount) {
					var queued = this.findSimilarJobs(job, { state: 'queued' });
					
					if (queued.length < queue_limit.amount) {
						// room in queue, yay!
						this.appendMetaLog(job, "Moving job state from {" + job.state + "} to {queued} (rate limit)");
						job.state = 'queued';
						job.position = job.priority ? 0 : (queued.length + 1); // updated every time a job changes state
						return false; // stop processing job launch
					}
						...
						// queue is full
						this.appendMetaLog(job, "Active jobs are blocking: " + jobs.map( job => '#' + job.id ).join(', ') );
						this.abortJob(job, "Job rate limit for event has been reached, and the queue is maxed out.");
						return false;
					}
				}
				else {
					// no can run
					this.appendMetaLog(job, "Active jobs are blocking: " + jobs.map( job => '#' + job.id ).join(', ') );
					this.abortJob(job, "Job rate limit for event has been reached, and queuing is disabled.");
					return false;
				}
			} // rate limit reached
		} // rate limit
	...
	updateJobLate(updates) {
		...
		this.unbase.update( 'jobs', updates.id, function(job) {
			...
			// remove things that should not be updated post-completion
		...
		function(err, job) {
			...
			self.doPageRestrictedBroadcast( 'Job?id=' + updates.id, 'job_completed', {}, job );
		} ); // unbase.update
	}
		...
		Object.values(data).forEach( function(updates) {
			...
			if (updates.procs) {
				...
				if (!info.timelines.minute.length || (info.timelines.minute[ info.timelines.minute.length - 1 ].epoch != cur_min_epoch)) {
					...
					// update page users
					self.doPageRestrictedBroadcast( 'Job?id=' + job.id, 'minute_append', item, job );
				} // minute
			} // procs
			...
			// progress updates get a direct line
			if (updates.progress && (updates.progress != job.progress)) {
				self.doPageRestrictedBroadcast( 'Job?id=' + job.id, 'job_progress', { progress: updates.progress }, job );
			}
	...
	applyJobCustomOverrides(job, info) {
		// allow config to specify a set of criteria matches + updates to be shallow-merged
		var self = this;
		var rules = this.config.get('job_custom_overrides') || [];
		if (!Array.isArray(rules) || !rules.length) return; // fast exit if no rules configured
		
		rules.forEach( function(rule) {
			if (!rule || !Tools.isaHash(rule)) {
				self.appendMetaLog(job, "WARNING: Malformed override rule (invalid rule syntax)");
				return;
			}
			if (!rule.expression || (typeof(rule.expression) != 'string')) {
				self.appendMetaLog(job, "WARNING: Malformed override rule (missing or invalid expression)");
				return;
			}
			if (!rule.updates || !Tools.isaHash(rule.updates)) {
				self.appendMetaLog(job, "WARNING: Malformed override rule (missing or invalid updates)");
				return;
			}
			
			var combo_job = Tools.mergeHashes( job, info );
			var expression = rule.expression;
			var updates = { ...rule.updates };
			
			// make sure update doesn't contain specific keys
			// (just stopping the user from shooting themselves in the foot)
			['data', 'files', 'input', 'activity', 'timelines', 'output', 'workflowData', 'serverData', 'id', 'state', 'completed', 'elapsed', 'event', 'category', 'plugin'].forEach( key => delete updates[key] );
			
			try {
				var result = jexl.evalSync( expression, combo_job );
				if (result) Tools.mergeHashInto(job, Tools.copyHash(updates, true));
			}
			catch (err) {
				self.appendMetaLog(job, "WARNING: Failed to process update rule: " + expression + ": " + (err.message || err) );
			}
			
			if (job.push && Tools.isaHash(job.push)) {
				for (var key in job.push) {
					if (Array.isArray(job[key])) job[key] = job[key].concat( job.push[key] );
				}
				delete job.push;
			}
		} ); // foreach item
	...
	finishJob(job) {
		...
		job.elapsed = Math.max(0, Tools.shortFloat( job.completed - job.started ));
		...
		// allow configuration to override job props based on criteria matches
		this.applyJobCustomOverrides(job, info);
		...
		this.prepJobLog( job, function() {
			...
			// check for retry
			if (job.code && !job.retried) self.checkRetryJob(job);
			
			// ensure all tags are proper
			if (!job.tags) job.tags = [];
			job.tags = job.tags.map( function(tag) {
				var tag_def = Tools.findObject( self.tags, { id: tag } );
				if (tag_def) return tag_def.id;
			...
			self.runJobActions(job, conditions, function() {
				...
				if (job.alerted) job.tags.push('_alerted');
				if (job.limited) job.tags.push('_limited');
				...
				// update daily stats if job was not retried
				if (!job.retried) {
				...
				var finish = function() {
					...
					self.doPageRestrictedBroadcast( 'Job?id=' + job.id, 'job_completed', {}, job );
	...
	uploadJobLog(job) {
		...
		this.storage.putStream( log_path, gzip, function(err) {
			if (err) self.logError('storage', "Failed to upload job log: " + log_path + ": " + err);
				...
				self.doPageRestrictedBroadcast( 'Job?id=' + job.id, 'log_uploaded', {}, job );
			}
	...
	checkRetryJob(job) {
		...
		// Allow xySat shutdown messages to enable retries
		// (this can be removed after xySat v1.0.42 is live)
		if ((job.code == 'abort') && (job.description == 'Server is shutting down.')) job.retry_ok = true;
		
		// if job was aborted, job.retry_ok or limit.force must be set for retry to fire
		if ((job.code == 'abort') && !job.retry_ok && !retry_limit.force) return;
		...
		if (job.retry_count < retry_limit.amount) {
			...
			for (var key in new_job) {
				if (!key.match(/^(type|event|category|plugin|targets|algo|workflow|input|params|parent|source|username|api_key|actions|limits|icon|label|test|test_conditions|retry_count|tags|invisible|ephemeral)$/)) delete new_job[key];
			}
			...
			// remove runtime action and limit props
			(new_job.actions || []).forEach( function(action) {
				delete action.date;
				delete action.elapsed_ms;
				delete action.code;
			...
			(new_job.limits || []).forEach( function(limit) {
				delete limit.date;
				delete limit.when;
				delete limit.code;
				delete limit.description;
				delete limit.details;
				delete limit.elapsed_ms;
				delete limit.msg;
			} );
