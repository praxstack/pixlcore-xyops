...
class Workflows {
	...
	runWFNode_event(opts) {
		...
		if (node.data.label) new_job.plabel = node.data.label;
	...
	runWFController_split(opts) {
		...
		// switch to unmananged mode if split is not wired to a single event or job node
		var goUnmanaged = function() {
			self.logWorkflow(job, node.id, `Switching to unmanaged mode with ${value.length} items and ${conns.length} destinations.`);
			...
			conns.forEach( function(conn) {
				...
				if (!dest) {
					// should never happen
					self.logWorkflow(job, node.id, `WARNING: Target node not found for split destination: #${conn.dest}`);
					return;
				}
		...
		if (conns.length > 1) {
			this.logWorkflow(job, node.id, `Split controller has multiple output connections (found ${conns.length})`);
			goUnmanaged();
			return;
		}
		var conn = conns[0];
		...
		if (!dest.type.match(/^(event|job)$/)) {
			this.logWorkflow(job, node.id, `Split controller is not wired to an event or job node.`);
			goUnmanaged();
			return;
		}
	...
	finishWorkflowJob(sub_job) {
		...
		if (sub_job.ephemeral) stub.deleted = true;
	...
	tickWFController_wait(opts) {
		...
		if ((now - state.start < node.data.wait) && !state.skip) return; // nope
