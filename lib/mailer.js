...
class FancyMailer {
	...
	sendFancyMail(name, args, callback) {
		...
		this.loadMailTemplate( name, function(err, template) { 
			...
			// save template html variables for later
			var temp_vars = {};
			template.toString().replace( /<\!--\s*(\w+)\:\s*(.*?)\s*-->\s*/g, function(m_all, m_g1, m_g2) {
				temp_vars[ m_g1.toLowerCase() ] = m_g2.trim();
				return '';
			} );
			
			// user may want a totally custom email body
			// note: args.body can still define its own To/Subject macros, and they will replace the ones being prepended
			if (args.body) {
				template = `<!-- To: ${temp_vars.to} -->\n<!-- Subject: ${temp_vars.subject} -->\n` + args.body;
			}
			...
			if (self.config.get('email_format') == 'html') {
				...
				opts.body = text.trim() + "\n\n" + opts.version + " (c) " + (new Date()).getFullYear() + ' ' + (self.config.getPath('client.company') || 'PixlCore LLC') + '.' + "\n";
			}
