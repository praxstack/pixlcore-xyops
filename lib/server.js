...
class ServerManager {
	...
	validateServer(server) {
		...
		if (server.id && this.servers[server.id] && (this.servers[server.id].hostname != server.hostname)) {
		...
		this.unbase.get( "servers", server.id, function(err, data) {
			...
			if (data) {
				...
				if (data.hostname && server.hostname && (data.hostname != server.hostname)) {
					self.logTransaction('warning', "Server hostname has changed: " + data.hostname + " to: " + server.hostname, server);
				}
				if (data.ip && server.ip && (data.ip != server.ip)) {
					self.logTransaction('warning', "Server IP address has changed: " + data.ip + " to: " + server.ip, server);
				}
			...
			if (server.autoGroup) {
				...
				if (self.manualAssignServerGroups(server)) {
					// manual groups changed (i.e. group deletion since last connect)
					if (!server.groups.length) {
						self.logDebug(9, "Switching server to autoGroup: " + server.id);
						server.autoGroup = true;
						self.autoAssignServerGroups(server);
					}
				}
			...
			if (!server.info.monitors) server.info.monitors = {};
			if (!server.info.deltas) server.info.deltas = {};
			...
			self.doUserBroadcastAllServers(true);
			...
			self.logTransaction('server_add', "Server connected to the network: " + server.hostname + " (" + server.ip + ")", {
				...
				ip: server.ip,
				nice_server: self.getNiceServerText(server),
	...
	removeServer(server) {
		...
		this.logTransaction('server_remove', "Server disconnected from the network: " + server.hostname + " (" + server.ip + ")", {
			hostname: server.hostname,
			ip: server.ip,
			nice_server: this.getNiceServerText(server),
			groups: server.groups,
			server_id: server.id,
			keywords: [ server.id ]
		});
		...
		// keep a copy of recent N removed servers in RAM, for client UX
		this.serverCache[ server.id ] = server;
		
		var server_cache_max_count = this.config.get('server_cache_max_count') || 100;
		while (Tools.numKeys(this.serverCache) > Math.max(0, server_cache_max_count)) {
		...
		// notify all users via ws
		this.doUserBroadcastAllServers(true);
	...
	deleteServer(server) {
		...
		// make sure server is gone from recent cache too
		delete this.serverCache[ server.id ];
	...
	manualAssignServerGroups(server) {
		// update manually-assigned server groups (groups may have been deleted)
		var self = this;
		var groups_changed = false;
		var old_groups = server.groups;
		
		server.groups = server.groups.filter( function(group_id) {
			if (!Tools.findObject(self.groups, { id: group_id } )) {
				groups_changed = true;
				return false;
			}
			return true;
		...
		if (groups_changed) {
			this.logDebug(9, "Manually-assigned groups for server have changed: " + server.id, { 
				id: server.id, 
				hostname: server.hostname, 
				'old': old_groups,
				'new': server.groups 
	...
	updateServerGroups(opts) {
		...
		for (var server_id in this.servers) {
			...
			if (server.autoGroup) {
				...
				// see if any manual servers need group-pruning
				if (this.manualAssignServerGroups(server)) {
					// server groups have changed, probably due to group getting deleted
					if (server.groups.length) {
						// server still has groups left
						this.doServerBroadcast( server_id, 'update', { groups: server.groups } );
					}
					else {
						// last remaining group was deleted, so we need to switch server to autoGroup
						this.logDebug(9, "Switching server to autoGroup: " + server.id);
						server.autoGroup = true;
	...
	getNiceServerText(server) {
		// get nice server string for notifications
		var nice_server = server.hostname;
		if (this.config.get('hostname_display_strip')) {
			nice_server = nice_server.replace( new RegExp(this.config.get('hostname_display_strip')), '' );
		}
		if (server.title) nice_server = server.title + ' (' + nice_server + ')';
		return nice_server;
	}
