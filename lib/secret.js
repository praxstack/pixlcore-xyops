...
class SecretUtils {
	...
	reloadSecretCache(callback) {
		// reload all encrypted secret records from storage into a fresh cache
		// always replace the old cache, even if one or more records fail to load,
		// so deleted or overwritten secrets can never linger in memory
		var self = this;
		var cache = {};
		var first_error = null;
		
		async.eachSeries( this.secrets,
			function(secret, callback) {
				self.storage.get( 'secrets/' + secret.id, function(err, record) {
					if (err) {
						if (!first_error) {
							first_error = new Error("Failed to load encrypted secret: " + secret.id + ": " + err);
						}
						return callback();
					}
					
					cache[ secret.id ] = record;
					callback();
				} );
			},
			function() {
				// swap the cache only after all storage reads have completed
				self.secretCache = cache;
	...
	getSecretsForJob(job) {
		...
		if (job.workflow && job.workflow.job && this.activeJobs[ job.workflow.job ]) {
			...
			if (wf_job.category != job.category) {
				Tools.mergeHashInto( env, this.getSecretsForType('categories', wf_job.category) );
			}
		}
