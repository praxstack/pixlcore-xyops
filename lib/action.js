...
class Actions {
	...
	incrementDailyJobStats(job, conditions) {
		// increment daily stats for job, for a specific set of conditions
		var self = this;
	...
	runJobActions(job, conditions, callback) {
		...
		// increment daily stats first, before filtering
		this.incrementDailyJobStats(job, conditions);
		...
		async.each( final_actions,
			...
			function() {
				if (self.shut) return setImmediate(callback); // ensure async
				...
				async.whilst(
					...
					function() {
						setImmediate(callback); // ensure async
					}
				); // whilst
			}
		); // async.each
	...
	runJobAction(job, action, callback) {
		...
		if (action.id && job.workflow && job.workflow.job) {
			wf_job = this.activeJobs[ job.workflow.job ];
		...
		this[func](job, action, function() {
			...
			if (wf_node_state) {
				...
				self.doPageRestrictedBroadcast( 'Job?id=' + job.workflow.job, 'action_append', { action }, wf_job || job );
			} // workflow
			
			// notify appropriate page to refresh action table
			self.doPageRestrictedBroadcast( 'Job?id=' + job.id, 'action_append', { action }, job );
	...
	runJobAction_run_event(job, action, callback) {
		...
		new_job.input = {
			data: Tools.copyHash( info.data || {}, true ),
			files: Tools.copyHash( info.files || [], true )
		};
		...
		// allow action to include previous job's raw output
		// (Note: Only works up to max_inline_job_log_size or 1MB)
		if (action.include_output) {
			new_job.input.data.text = info.output || '';
		}
		...
		// preserve caller workflow params in parent for the child's launch-time macros
		// workflow jobs use their own params, even when nested in another workflow
		// keep workflow reserved for actual workflow definitions and membership
		if (job.workflow) {
			new_job.parent.params = Tools.copyHash(
				(job.type == 'workflow' ? job.params : job.workflow.params) || {}, true
			);
		}
	...
	runJobAction_channel(job, action, callback) {
		...
		async.parallel(
			[
				function(callback) {
					// email
					if ((!channel.users || !channel.users.length) && !channel.email) return callback();
	...
	runJobAction_plugin(job, action, callback) {
		...
		var child = cp.exec( child_cmd, child_opts, function(err, stdout, stderr) {
			...
			if (json && json.xy) {
				...
				// allow plugin to merge data into job
				if (json.data && Tools.isaHash(json.data) && self.jobDetails[job.id]) {
					var info = self.jobDetails[ job.id ];
					if (action.condition == 'start') {
						if (!info.input) info.input = {};
						if (!info.input.data) info.input.data = {};
						Tools.mergeHashInto( info.input.data, json.data );
					}
					else {
						if (!info.data) info.data = {};
						Tools.mergeHashInto( info.data, json.data );
					}
				}
			}
	...
	fireSystemHook(action, orig_data) {
		...
		if (hook) {
			...
			// Login activity retains this credential internally for session
			// management, but no outgoing hook or template should receive it.
			...
			if (hook.email) {
				...
				// HTTP headers belong in the sanitized activity JSON, not the
				// mailer's custom MIME headers.  Keep that API available to send_email.
				var mail_args = Tools.copyHashRemoveKeys(data, { headers: 1 });
				this.sendFancyMail( 'activity', mail_args, function(err, body, log) {
