...
class Monitoring {
	...
	setupMonitoring(callback) {
		...
		// recompile on config reload (for quickmon)
	...
	precompileMonitors() {
		...
		// quickmon too
		this.config.get('quick_monitors').forEach( function(monitor) {
			self.logMonitor(9, "Pre-compiling quickmon source expression: " + monitor.id + ": " + monitor.source);
			try { expCache[ monitor.id ] = jexl.compile( monitor.source ); }
			catch(err) {
				self.logError('monitor', "Failed to pre-compile quickmon source: " + monitor.id + ": " + err);
				expCache[ monitor.id ] = jexl.compile("0"); // default to zero
			}
		} );
	}
	...
	handleMonitoringData(socket, params) {
		...
		var nice_server = this.getNiceServerText(server);
		...
		async.series(
			[
				...
				function(callback) {
					...
					Object.values(self.warmAlerts).forEach( function(alert) {
						...
						if ((alert.count == alert_def.samples) && !alert.notified) {
							...
							if (alert_def.enabled && !alerts_snoozed) {
							...
							self.logTransaction('alert_new', "New alert for: " + nice_server + ": " + alert_def.title, { 
								server: server.id,
								hostname: server.hostname,
								ip: server.ip,
								alert: global_alert,
							...
							jobs.forEach( function(job) {
								if (job.workflow && job.workflow.job && !global_alert.jobs.includes(job.workflow.job)) {
									...
									// add alerted flag to workflow
									var wf_job = self.activeJobs[ job.workflow.job ];
									if (wf_job) wf_job.alerted = true;
								}
								
								// add alerted flag, for adding tag later
								job.alerted = true;
							});
							...
							if (global_alert && alert_def.enabled && !alerts_snoozed) {
								notifications.push({
							...
							self.logTransaction('alert_cleared', "Alert cleared: " + nice_server + ": " + alert_def.title, { 
								server: server.id,
								hostname: server.hostname,
								ip: server.ip,
								alert: global_alert || alert,
								loc: global_alert ? `Alerts?id=${global_alert.id}` : ''
							});
				...
				function(callback) {
					...
					if (alerts_changed) {
						self.doUserBroadcastAllActiveAlerts();
					}
					...
					self.doPageRestrictedBroadcast('Servers?id=' + server.id, 'snapshot', { id: server.id, snapshot: params }, server);
					
					server.groups.forEach( function(group_id) {
						self.doPageRestrictedBroadcast('Groups?id=' + group_id, 'snapshot', { id: server.id, snapshot: params }, { groups: [group_id] });
					});
	...
	clearActiveAlert(id) {
		...
		// load latest server monitoring data
		var host_key = 'hosts/' + server.id + '/data';
	...
	cleanupGlobalAlerts() {
		...
		if (alerts_changed) {
			this.doUserBroadcastAllActiveAlerts();
		}
	}
	...
	handleQuickMonData(socket, data) {
		...
		mon_defs.forEach( function(def) {
			// evaluate source using params.data as context
			var exp = self.expressionCache[ def.id ];
			if (!exp) return; // sanity
			
			var value = 0;
			try { value = exp.evalSync( data ); }
			catch (err) {
				self.logMonitor(7, "Quickmon expression failed to evaluate: " + def.source + ": " + err, {
					quickmon: def,
					hostname: server.hostname,
					server: server.id
				});
			...
			// support custom data_match regexp to extract value out of string
			if (def.data_match) {
				var matches = ('' + value).match( def.data_match );
				if (matches && (matches.length >= 2)) {
					// data_match has a group capture, use first group
					value = matches[1];
				}
					...
					self.logMonitor(7, "Quickmon data regexp did not match: " + def.data_match, {
						quickmon: def,
						server: server.id,
						hostname: server.hostname,
						raw_value: value
		...
		this.doPageRestrictedBroadcast('Servers?id=' + server.id, 'quickmon', { id: server.id, row: stub, data: data }, server);
		
		server.groups.forEach( function(group_id) {
			self.doPageRestrictedBroadcast('Groups?id=' + group_id, 'quickmon', { id: server.id, row: stub, data: data }, { groups: [group_id] });
		});
	}
