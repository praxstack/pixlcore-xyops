...
class Communication {
	...
	handleNewSocket(web_socket, req) {
		...
		var socket = {
			...
			send: function(cmd, data) {
				self.logComm(10, "Sending socket message: " + cmd);
				
				if (this.ws.readyState !== WebSocket.OPEN) {
					self.logError('socket', "Socket not connected, message not sent", { cmd });
					return false;
				}
				...
				try {
					this.ws.send(
						...
						function(err) {
							...
							if (err) {
								self.logError('socket', "Failed to send socket message: " + cmd + ": " + err);
							}
						}
					);
				}
		...
		web_socket.on('close', function() {
			...
			// if user disconnected, refresh nav data in all clients on next tick
			if (socket.auth && (socket.type == 'user')) self.navChanged = true;
		} );
	}
	...
	handleSocketMessage(socket, json) {
		...
		var self = this;
		this.logComm(10, "Got message from socket: " + socket.id + ": " + json.cmd);
		...
		switch(cmd) {
			...
			case 'join':
				...
				// de-auth and close any stale sockets for server (reconnect race condition)
				Object.values(this.sockets).forEach( function(sock) {
					if (sock.id == socket.id) return; // skip current socket
					if (sock.auth && sock.server && (sock.server.id == socket.server.id)) {
						self.logComm(5, "Closing existing socket for server: " + sock.id, { server: sock.server.id, hostname: sock.server.hostname });
						delete sock.server;
						delete sock.auth;
						sock.close();
					}
				...
				// join the network
				this.addServer(socket.server, function() {
					// make sure server has vital data
					socket.send('joined', {
						config: Tools.mergeHashes( { airgap: self.config.get('airgap') }, self.config.getPath('satellite.config') ),
						...
						numServers: Tools.numKeys(self.servers),
						features: {
							// tell server which features we support (xyops v1.0.15+)
							api_finish_job: true,
							disable_self: true
						}
					});
			...
			case 'authenticate':
				...
				this.logComm(9, "Incoming socket auth request from client", data);
				...
				this.loadSession( args, function(err, session, user) {
					...
					var computed = self.getComputedUser(user);
					socket.admin = !!(computed.privileges.admin);
					socket.computed = computed;
					...
					// update essential client data if this is a reconnect
					if (data.attempt && (data.attempt > 1)) {
						self.logComm(6, `Socket reconnected (${data.attempt}), sending full client data update`);
						socket.send( 'update', {
							activeJobs: self.getUserLimitedActiveJobs(computed),
							internalJobs: self.getInternalJobs(),
							numQueuedJobs: self.countQueuedJobs(),
							socketNav: self.getSocketNavData(),
							activeAlerts: self.getUserLimitedActiveAlerts(computed),
							serverCache: self.getUserLimitedCachedServers(computed),
							servers: self.getUserLimitedActiveServers(computed),
							groups: self.getUserLimitedGroups(computed),
							monitors: self.monitors,
							alerts: self.alerts,
							categories: self.getUserLimitedCategories(computed),
							channels: self.channels,
							web_hooks: self.web_hooks,
							buckets: self.buckets,
							secrets: self.secrets, // just metadata, no actual secrets in here
							events: self.getUserLimitedEvents(computed),
			...
			case 'jobs':
				...
				if (socket.auth && (socket.type == 'server')) this.updateJobData(data);
			break;
			...
			case 'disable':
				// allow server to disable itself, for graceful shutdown
				if (socket.auth && (socket.type == 'server') && this.servers[socket.server.id]) {
					this.logComm(5, "Server is disabling itself: " + socket.server.id );
					var server = this.servers[socket.server.id];
					server.enabled = false;
					this.doUserBroadcastAllServers();
				}
			break;
	...
	doPageRestrictedBroadcast(loc, cmd, data, criteria) {
		// only broadcast to user sockets with appropriate limits
		// criteria can be any object with category and/or targets and/or groups (e.g. event, job, server, alert, etc.)
		if (this.shut) return;
		if (typeof(loc) == 'string') loc = { loc: loc, id: loc.replace(/\?.*$/, ''), query: Tools.parseQueryString(loc) };
		...
		Object.values(this.sockets).forEach( socket => {
			if (!socket.auth || !socket.user || !socket.loc || !this.compLoc(socket.loc, loc)) return;
			
			var cuser = socket.computed;
			
			if ('category' in criteria) {
				if (cuser.categories.length && (!criteria.category || !cuser.categories.includes(criteria.category))) return;
			}
			
			// Group limits are a legacy event feature, so workflows are only
			// restricted by their top-level category for read broadcasts.
			if ((criteria.type != 'workflow') && (('targets' in criteria) || ('groups' in criteria))) {
				var groups = criteria.targets || criteria.groups || [];
				if (cuser.groups.length && (!groups.length || !Tools.includesAny(cuser.groups, groups))) return;
			}
			
			if (criteria.privilege && !cuser.privileges[criteria.privilege] && !cuser.privileges.admin) return;
	...
	doUserBroadcastAllEvents() {
		// broadcast all events to all users, with user limits applied
		if (this.shut) return;
		
		for (var id in this.sockets) {
			var socket = this.sockets[id];
			if (socket.auth && socket.user && socket.computed) {
				socket.send( 'update', {
					events: this.getUserLimitedEvents(socket.computed)
				} );
			}
		}
	}
	
	doUserBroadcastAllCategories() {
		// broadcast all categories to all users, with user limits applied
		if (this.shut) return;
		...
		for (var id in this.sockets) {
			...
			if (socket.auth && socket.user && socket.computed) {
				socket.send( 'update', {
					categories: this.getUserLimitedCategories(socket.computed)
				} );
			}
		}
	}
	
	doUserBroadcastAllGroups(inc_servers) {
		// broadcast all groups to all users, with user limits applied
		if (this.shut) return;
		...
		for (var id in this.sockets) {
			...
			if (socket.auth && socket.user && socket.computed) {
				var data = {
					groups: this.getUserLimitedGroups(socket.computed)
				};
				if (inc_servers) {
					data.servers = this.getUserLimitedActiveServers(socket.computed)
				}
	...
	doUserBroadcastAllServers(inc_cached) {
		// broadcast all servers to all users, with user limits applied
		if (this.shut) return;
		...
		for (var id in this.sockets) {
			...
			if (socket.auth && socket.user && socket.computed) {
				var data = {
					servers: this.getUserLimitedActiveServers(socket.computed)
				};
				if (inc_cached) {
					data['serverCache'] = this.getUserLimitedCachedServers(socket.computed);
				}
				socket.send( 'update', data );
			}
		}
	...
	doUserBroadcastAllActiveAlerts() {
		// broadcast all servers to all users, with user limits applied
		if (this.shut) return;
		...
		for (var id in this.sockets) {
			...
			if (socket.auth && socket.user && socket.computed) {
				socket.send( 'update', {
					activeAlerts: this.getUserLimitedActiveAlerts(socket.computed)
				} );
			}
		}
	}
	...
	sendSocketPings() {
		...
		for (var id in this.sockets) {
			...
			else if (!socket.disconnected && !socket.auth && (now - socket.timeStart >= 30)) {
				// close old sockets that haven't auth'ed in first 30s
				this.logComm(5, "Closing no-auth stale socket: " + socket.id, { ip: socket.ip });
				socket.ws.terminate();
			}
	...
	getSocketNavData() {
		// get serializable structure containing all socket nav data
		var data = {};
		
		for (var id in this.sockets) {
			var socket = this.sockets[id];
			if (socket.auth && (socket.type == 'user') && socket.username && socket.loc) {
				data[ socket.id ] = { username: socket.username, loc: socket.loc };
			}
		}
