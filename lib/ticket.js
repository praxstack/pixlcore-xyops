...
class TicketSystem {
	...
	processDelayedTicketChange(args) {
		...
		this.unbase.get( 'tickets', record_id, function(err, ticket) {
			...
			var usernames = {};
			if (ticket.username) usernames[ ticket.username ] = 1;
	...
	getTicketDisplayArgs(args) {
		...
		// display.username
		if (ticket.username) {
			var user = args.users[ ticket.username ];
			display.username = user ? user.full_name : `(${ticket.username})`
		}
	...
	sendTicketEmailNotification(args) {
		...
		var encodeBraces = function(md) {
			// do not allow body/comments to eval nested macros
			return String(md).replace(/\{\{/g, '{ {').replace(/\}\}/g, '} }');
		};
		
		ticket.subject = encodeBraces(ticket.subject);
		ticket.body = encodeBraces(ticket.body);
