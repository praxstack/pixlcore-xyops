...
class xyOps extends Component {
	...
	earlyStart() {
		...
		['base_app_url', 'secret_key', 'log_dir', 'log_filename', 'temp_dir', 'log_columns', 'debug_level'].forEach( function(path) {
		...
		// hook logger, to scan for errors
	...
	finishStartup(callback) {
		...
		// health (master) check
		this.web.addURIHandler( '/health', "Health Check", false, function(args, callback) {
	...
	tick() {
		...
		if (this.numSocketClients) {
			if (this.master) this.sendUserStatusUpdateAll();
	...
	sendUserStatusUpdateAll() {
		// send status updates to all users
		// each user may get a different set of active jobs
		var status = {
			epoch: Tools.timeNow(),
			internalJobs: this.getInternalJobs(),
			numQueuedJobs: this.countQueuedJobs(),
			jobRateLimits: this.jobRateLimits
		};
		if (this.jobsChanged) {
			status.jobsChanged = true;
			status.state = Tools.copyHashRemoveKeys( this.state, { dirty: 1 } );
			delete this.jobsChanged;
		}
		if (this.navChanged) {
			status.navChanged = true;
			status.socketNav = this.getSocketNavData();
			delete this.navChanged;
		}
		
		for (var id in this.sockets) {
			var socket = this.sockets[id];
			if (socket.auth && socket.user && socket.computed) {
				status.activeJobs = this.getUserLimitedActiveJobs(socket.computed);
	...
	beforeUserLogin(args, callback) {
		...
		var cuser = this.getComputedUser(args.user);
		
		args.resp = {
			epoch: Tools.timeNow(),
			activeJobs: this.getUserLimitedActiveJobs(cuser),
			internalJobs: this.getInternalJobs(),
			numQueuedJobs: this.countQueuedJobs(),
			socketNav: this.getSocketNavData(),
			activeAlerts: this.getUserLimitedActiveAlerts(cuser),
			serverCache: this.getUserLimitedCachedServers(cuser),
			servers: this.getUserLimitedActiveServers(cuser),
			groups: this.getUserLimitedGroups(cuser),
			monitors: this.monitors,
			alerts: this.alerts,
			categories: this.getUserLimitedCategories(cuser),
			channels: this.channels,
			web_hooks: this.web_hooks,
			buckets: this.buckets,
			secrets: this.secrets, // just metadata, no actual secrets in here
			events: this.getUserLimitedEvents(cuser),
		...
		// copy all API keys
		var keys = Tools.copyHash( this.api_keys, true );
	...
	beforeUserCreate(args, callback) {
		...
		// administrators are never category or group limited, including users
		// who inherit the administrator privilege from one of their roles
		if (this.getComputedPrivileges(user).admin) {
			user.categories = [];
			user.groups = [];
		}
	...
	beforeUserUpdate(args, callback) {
		...
		// Evaluate the final merged user so this also catches an update that grants
		// administrator directly, or assigns a role which grants administrator.
		// Mutate both objects because the user manager has several save paths: its
		// normal APIs apply `updates` later, while SSO saves may persist `user`.
		var updated_user = Tools.mergeHashes( user, updates );
		if (this.getComputedPrivileges(updated_user).admin) {
			user.categories = [];
			user.groups = [];
			updates.categories = [];
			updates.groups = [];
		...
		if (updates.new_password) {
			...
			// delete force password flag
			delete user.force_password_change;
		}
	...
	afterUserChange(action, args) {
		...
		if (action == 'user_update') {
			for (var id in this.sockets) {
				var socket = this.sockets[id];
				if (socket.auth && socket.user && (socket.username == username)) {
					var computed = this.getComputedUser(args.user);
					...
					if (!args.user.active) {
						...
						// push update of all data that may be changed by user category/group limits
						socket.send( 'update', {
	...
	logActivity(action, orig_data) {
		...
		var self = this;
		if (!this.master) return;
		if (this.server.shut) return; // sanity
		...
		this.unbase.insert( 'activity', data.id, data, function(err) {
			...
			else self.doPageRestrictedBroadcast( 'ActivityLog', 'activity', { data }, { privilege: 'admin' } );
		} );
	...
	shutdown(callback) {
		...
		if (this.launchQueue) this.launchQueue.kill();	
		if (this.searchQueue) this.searchQueue.kill();
...
ClassUtil.mixin( xyOps, [
	...
	require('./multi.js'),    // Multi-Conductor Layer Mixin
